...
cs.Add("/home/zambra/root_files/clusterdata/out/GetSimpleTuple_HSim/C9_yshiftm03/prunedC_*.root");
   ...
   float x,y,k,l,q,w,xb,yb;
   float xd,yd,kd,ld,qd,wd,xbd,ybd;
   ...
   cs.SetBranchAddress("mc_Yb", &yb);
    ...
    TH1F *hd = new TH1F("hd","P for +Pions, W>2  Q^{2}>1 X_{f}>0.1 X_{b}<1, Y_{b}<0.85",1000,0,2);
    TH1F *hs = new TH1F("hs","P for +Pions, W>2  Q^{2}>1 X_{f}>0.1, Y_{b}<0.85",1000,0,2);
   ...
   leg->Draw("SAME");
   gStyle->SetOptStat(0);
   c1->SaveAs("P_Pion_C_norm_cuts_q_w_xf_xb_yb.pdf");
