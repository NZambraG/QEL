...
void E_distribution_ACC_NoACC()
{
    ...
    TH1F *hdmc = new TH1F("hmc"," ",100,0,2.7);

    TH1F *hcd = new TH1F("hmc"," ",100,0,2.7);
    TH1F *hcmc = new TH1F("hmc"," ",100,0,2.7);
...
//C
    ...
    hdd->Scale(1./hd->Integral());
    hcd->Scale(1./hd->Integral());
...
//modified D
    ...
    hdmc->Scale(1./hdmc->Integral());
    ks2 = hdd->KolmogorovTest(hdmc);

// corrected modified D
    for (size_t irow=0; irow<cc.GetEntries(); ++irow){
        c.GetEntry(irow);
      ...
      if(ttc==1){
         if (lc==211)
         {
            ...
            sr1cc=sqrt((pc+dE)*(pc+dE)-(1+(dE/pc))*(1+(dE/pc))*pt2cc);
            sr2cc=sqrt((wc*wc-mn*mn+mppion*mppion)*(wc*wc-mn*mn+mppion*mppion)-4*wc*wc*mppion*mppion);
            sr3cc=sqrt(qcc*qcc+nuc*nuc);
            mxfcc=2*((nuc+mp)*(sr1cc-((sr3cc*(zhc*nuc+dE))/(nuc+mn))))/(sr2cc);
      if (mxfcc>0.1)
      {
            if (qcc>1)
         {
