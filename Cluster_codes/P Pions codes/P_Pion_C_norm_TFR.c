...
cs.Add("/eos/user/n/nzambra/out/GetSimpleTuple_HSim/C9_yshiftm03/prunedC_*.root");
    ...
    TH1F *hd = new TH1F("hd","P for +Pions, W>2, Q^{2}>1, mod X_{f}<0, X_{b}<1, Y_{b}<0.85",500,0,2.7);
   ...
   leg->Draw("SAME");
   gStyle->SetOptStat(0);
   c1->SaveAs("P_Pion_C_norm_q_w_mxf_for_TFR_xb_yb_500bin.pdf");
