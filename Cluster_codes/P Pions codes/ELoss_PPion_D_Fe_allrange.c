...
void ELoss_PPion_D_Fe_allrange()
{
    ...
    TH1F *hd = new TH1F("hd","Energy loss for +Pions Fe, w>2  Q^{2}>1 mod X_{f}>0.1  X_{b}<1",250,1,2.7);
...
//modified D
 ...
 Double_t ks = hd->KolmogorovTest(hc);
  Double_t ks2 = hd->KolmogorovTest(hmc);
    ...
    leg->Draw("SAME");
    gStyle->SetOptStat(0);
    c1->SaveAs("ELoss_Pion_D_Fe_modD_160mev_bin250_range1_27.pdf");
}
