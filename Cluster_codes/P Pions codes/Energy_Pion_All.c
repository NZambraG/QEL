...
void Energy_Pion_All()
{
    ...
    TH1F *hc = new TH1F("hc","Energy for Pion^{+} using D reconstructed from every material data  , w>2  Q^{2}>1 mod X_{f}>0.1  X_{b}<1",100,0,2);
    TH1F *hd = new TH1F("hd"," ",100,0,2);   
...
//Fe
   ...
   //Pb
    ...
    leg->Draw("SAME");
    gStyle->SetOptStat(0);
    c1->SaveAs("P_Pion_D_C_Fe_Pb_norm_CFR01_100bin.pdf");
